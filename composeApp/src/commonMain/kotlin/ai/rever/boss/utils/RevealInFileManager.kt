 ...
 *
 * Security: Validates that the path is within the allowed boundary
...
fun revealInFileManager(path: String): Result<Unit> {
    ...
    val execResult = executeRevealCommand(file)
    ...
    if (execResult.isFailure) {
        revealLogger.warn(
            ...
            mapOf("path" to path),
            error = execResult.exceptionOrNull(),
        )
    }
...
private fun executeRevealCommand(file: File): Result<Unit> =
    runCatching {
        if (osName.contains("mac")) {
            Runtime.getRuntime().exec(arrayOf("open", "-R", file.absolutePath))
        } else if (osName.contains("win")) {
            executeWindowsReveal(file)
        } else {
            executeLinuxReveal(file)
        }
        Unit
    }

private fun executeWindowsReveal(file: File) {
    ...
    if (winPath.contains("  ")) {
        ...
        Runtime.getRuntime().exec(
            arrayOf(
                "explorer.exe",
                (file.parentFile ?: file).absolutePath,
            ),
        )
    } else {
...
private fun executeLinuxReveal(file: File) {
