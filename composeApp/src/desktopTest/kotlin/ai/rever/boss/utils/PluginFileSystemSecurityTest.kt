...
class PluginFileSystemSecurityTest {
    ...
    fun `normalizes relative paths correctly`() =
        runTest {
            ...
            val testFile = File(testDir, "test.txt").apply { writeText("test content") }
            val relativePath = "$homeDir/./plugin-security-test/test.txt"
            val result = PluginFileSystemSecurity.validateAndNormalizePath(relativePath, "test")
            // Result should be a valid path within allowed roots
            assertTrue(result.contains("plugin-security-test"))
    ...
    fun `prevents symlink escape from allowed directory`() =
        runTest {
            ...
            // Use a real path that exists on the system
            val symlinkTarget =
                if (System.getProperty("os.name").startsWith("Windows", ignoreCase = true)) {
                    "C:\\Windows"
                } else {
            ...
            // Try to access the symlink itself - it should be denied because the target is outside allowed roots
    ...
    fun `allows symlink within allowed directory`() =
        runTest {
            ...
            // The result should resolve to the actual target path
            val result = PluginFileSystemSecurity.validateAndNormalizePath(symlink.absolutePath, "test")
            // Result should be a valid path within allowed roots (resolves to target)
            assertTrue(result.contains("plugin-security-test"))
            assertTrue(result.contains("target.txt"))
        }
    ...
    fun `prevents case-sensitivity bypass attempts`() =
        runTest {
            ...
            // don't bypass security checks. On Linux (case-sensitive), this just verifies
            // that the security layer accepts valid paths.
            val testFile = File(testDir, "test.txt").apply { writeText("test content") }

            // The security layer normalizes paths to canonical form
            val result = PluginFileSystemSecurity.validateAndNormalizePath(testFile.absolutePath, "test")
            // Result should be a valid path within allowed roots
            assertTrue(result.contains("plugin-security-test"))
            assertTrue(result.contains("test.txt"))
