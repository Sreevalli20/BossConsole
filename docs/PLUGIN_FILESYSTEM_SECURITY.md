...
Note: The current project directory is not currently passed during initialization but can be added via `PluginFileSystemSecurity.addAllowedRoot()` when available.
...
- **Real Path Resolution**: All paths are resolved using the nearest-existing-ancestor pattern from FileSystemPathPolicy
- **Root-Based Checks**: Paths must be within at least one of the explicitly granted roots
- **Symlink Resolution**: Symlinks are resolved to their targets using `toRealPath()` before boundary checks
...
4. **Real Path Resolution**: For existing paths, uses `toRealPath()` to resolve symlinks and platform-specific path issues
5. **Nearest-Existing-Ancestor Resolution**: For non-existent paths, finds the nearest existing ancestor, resolves it with `toRealPath()`, then re-resolves the remaining path components

### Traversal Prevention

Path traversal attacks are prevented through:

- **Real Path Comparison**: Uses `toRealPath()` for boundary checks (not string comparison)
- **Segment Validation**: Validates child names don't contain path separators or traversal sequences
- **Symlink Safety**: Resolves symlinks using `toRealPath()` before boundary checks to prevent symlink escapes
- **Component-Aware Path Checks**: Uses `Path.startsWith()` which is component-aware to prevent sibling-prefix attacks
...
1. **FileSystemDataProviderImpl**: All filesystem operations now validate paths
   ...
   - `rename`
   - `readFile` / `writeFile`
   - `revealInFileManager` (validates paths using PluginFileSystemSecurity before delegating to RevealInFileManager utility)

2. **RevealInFileManager**: The reveal utility performs security validation using PluginFileSystemSecurity before executing OS-specific reveal commands. This ensures that plugins cannot reveal sensitive system files through the file manager.
...
- Workspaces outside $HOME that are explicitly granted (e.g., through project selection or user-mediated file pickers)

Note: The explicit capability model supports legitimate workspaces outside $HOME when they are explicitly granted. This is more flexible than a strict $HOME-only boundary but requires host-mediated grants for non-home directories.
...
Symlinks are resolved to their targets using `toRealPath()` before boundary checks, following the FileSystemPathPolicy pattern. This prevents symlink escapes but means that plugins can access files through symlinks if the target is within granted roots. For non-existent paths, the nearest-existing-ancestor resolution ensures consistent behavior even when parent directories contain symlinks.
